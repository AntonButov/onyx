...
def _form_channel_config(
    ...
    if respond_tag_only and respond_member_group_list:
        raise ValueError(
            "Cannot set CleardocsBot to only respond to tags only and also respond to a predetermined set of users."
        )
    ...
    if (
        ...
        raise ValueError(
            "Cannot set CleardocsBot to respond to users in a private (ephemeral) message "
