...
- **Product icon / emblem (widget vs web):** The embeddable chat widget uses **`widget/public/logo.png`** (built into `widget/dist/onyx-widget.js`). The main Next.js UI at **`http://localhost:3000`** uses separate files: **`web/public/cleardocs-emblem.png`**, **`web/src/app/icon.png`**, **`web/src/app/apple-icon.png`**. Changing only the widget file does not update the web app. After updating `widget/public/logo.png`, run **`node scripts/sync-web-brand-icons-from-widget.mjs`** from the repo root, then rebuild/restart **`web_server`** (see Docker note below) so the new icons are in the container.
- **Product icon / emblem (widget vs web):** The embeddable chat widget uses **`widget/public/logo.png`** (built into `widget/dist/onyx-widget.js`). The main Next.js UI at **`http://localhost:3000`** uses separate files: **`web/public/cleardocs-emblem.png`**, **`web/src/app/icon.png`**, **`web/src/app/apple-icon.png`**. Changing only the widget file does not update the web app. After updating `widget/public/logo.png`, run **`node scripts/sync-web-brand-icons-from-widget.mjs`** from the repo root, then rebuild/restart **`web_server`** (see Docker note below) so the new icons are in the container.
